var GithubIssue = (function(){
  ...
  // render repository items
  var reposDataHandler = function(repoArray){
    var htmlRepoString = ''
    if(repoArray.length < 1){
      showAlert('Not able to find any repository');
      return false;
    }
...
var showAlert = function (msg){
  $('#gm-alert').removeClass('d-none').addClass('d-block').html(msg);
  window.scrollTo(0,0);
  setTimeout(function(){
    $('#gm-alert').addClass('d-none').removeClass('d-block');
  },3000)
}
...
var createIssue = function(){
  ...
  $.ajax({
    ...
    success: function (response) {
      $('.title, .desc').val('');
      showAlert('Issue has been created successfully!');
    },
    error: function () {
      showAlert('some error occured while creating issue!');
    },
  });
};
  // init event bindings
